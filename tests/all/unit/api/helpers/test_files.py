...
class TestFilesMethods(unittest.TestCase):
    ...
    def test_frontend_url(self):
        ...
        with patch('app.api.helpers.files.get_settings') as get_settings:
            ...
            patch_settings(get_settings, 'https://next.theeventcity.com/')
            self.assertEqual('https://next.theeventcity.com/verify?token=ok_this_is_a_secret',
                             make_frontend_url('/verify', {'token': 'ok_this_is_a_secret'}))

            self.assertEqual('https://next.theeventcity.com/verify?token=ok_this_is_a_secret',
                             make_frontend_url('/verify', {'token': 'ok_this_is_a_secret'}))

            self.assertEqual('https://next.theeventcity.com/verify',
