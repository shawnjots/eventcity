...
* **Step 0** - Clone the Events City Server repository (from the development branch) and ```cd ``` into the directory.
```sh
...
You can use either **pip** or **pipenv** to install Events City Server in a virtual environment.
